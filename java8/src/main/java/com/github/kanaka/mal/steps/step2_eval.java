...
public class step2_eval {
	...
	public static void main(String[] args) {
		...
		try {
			...
			repl_env.set(symbol("+"), fn(step2_eval::add));
			repl_env.set(symbol("-"), fn(step2_eval::subtract));
			repl_env.set(symbol("*"), fn(step2_eval::multiply));
			repl_env.set(symbol("/"), fn(step2_eval::divide));
			while (true) {
